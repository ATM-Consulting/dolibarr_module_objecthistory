...
$hookmanager->initHooks(array('objecthistorylist'));
...
llxHeader('',$langs->trans('ObjectHistoryList'),'','');

//$type = GETPOST('type');
//if (empty($user->rights->objecthistory->all->read)) $type = 'mine';

// TODO ajouter les champs de son objet que l'on souhaite afficher
$sql = 'SELECT t.rowid, t.ref, t.label, t.date_creation, t.tms, \'\' AS action';

$sql.= ' FROM '.MAIN_DB_PREFIX.'objecthistory t ';

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('ObjectHistory', 1).')';
//if ($type == 'mine') $sql.= ' AND t.fk_user = '.$user->id;


$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_objecthistory', 'GET');

$nbLine = !empty($user->conf->MAIN_SIZE_LISTE_LIMIT) ? $user->conf->MAIN_SIZE_LISTE_LIMIT : $conf->global->MAIN_SIZE_LISTE_LIMIT;

$r = new Listview($db, 'objecthistory');
echo $r->render($sql, array(
	...
	,'search' => array(
		...
		,'status' => array('search_type' => ObjectHistory::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
		'rowid'
	...
	,'list' => array(
		'title' => $langs->trans('ObjectHistoryList')
		,'image' => 'title_generic.png'
		,'picto_precedent' => '<'
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('NoObjectHistory')
